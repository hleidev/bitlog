...
class ImageCleanupTaskTest {
    ...
    @BeforeEach
    void setUp() {
        imageCleanupTask = new ImageCleanupTask(
                fileService, new ArticleImageReferenceServiceImpl(articleVersionDAO, fileUrlHelper));
    }
    ...
    void cleanOrphanImages_noTrackedKeys_returnsEarlyWithNoDeletes() {
        ...
        verify(fileService, never()).deleteAndConfirm(anyString());
    ...
    void cleanOrphanImages_unreferencedKeys_areDeletedAndMarked() {
        when(fileService.deleteAndConfirm(anyString())).thenReturn(true);
        ...
        verify(fileService, times(1)).deleteAndConfirm(orphanKey);
        verify(fileService, never()).deleteAndConfirm(referencedKey);
    ...
    void cleanOrphanImages_allTrackedKeysAreOrphans_allDeletedAndMarked() {
        ...
        verify(fileService, times(1)).deleteAndConfirm(key1);
        verify(fileService, times(1)).deleteAndConfirm(key2);
    ...
    void cleanOrphanImages_markDeletedCalledOnceForAllOrphans() {
        ...
        verify(fileService, times(2)).deleteAndConfirm(anyString());
        verify(fileService, times(1)).markDeleted(anyCollection());
    }

    @Test
    void cleanOrphanImages_partialDeleteFailure_marksOnlySuccessAndRetriesFailure() {
        String success = "article/1/success.png";
        String failure = "article/1/failure.png";
        when(fileService.getOldUndeletedContentKeys(any())).thenReturn(List.of(success, failure), List.of(failure));
        stubContents(List.of());
        when(fileService.deleteAndConfirm(success)).thenReturn(true);
        when(fileService.deleteAndConfirm(failure)).thenReturn(false, true);

        imageCleanupTask.cleanOrphanImages();
        verify(fileService).markDeleted(List.of(success));
        verify(fileService, never()).markDeleted(List.of(success, failure));

        imageCleanupTask.cleanOrphanImages();
        verify(fileService).markDeleted(List.of(failure));
        verify(fileService, times(2)).deleteAndConfirm(failure);
    }

    @Test
    void cleanOrphanImages_allDeletesFail_doesNotMarkDeleted() {
        when(fileService.getOldUndeletedContentKeys(any())).thenReturn(List.of("article/1/failure.png"));
        stubContents(List.of());
    ...
    private void stubContents(List<String> contents) {
        doAnswer(invocation -> {
                ...
                .when(articleVersionDAO)
                .forEachActiveContent(any());
    }
}

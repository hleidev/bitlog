...
class MyArticlePageParamTest {
    ...
    void getPublishedOnly_statusPublished_returnsTrue() {
        assertThat(new MyArticlePageParam().setStatus(ArticleStatusEnum.PUBLISHED).getPublishedOnly()).isTrue();
    }

    @Test
    @DisplayName("草稿映射为 false")
    void getPublishedOnly_statusDraft_returnsFalse() {
        assertThat(new MyArticlePageParam().setStatus(ArticleStatusEnum.DRAFT).getPublishedOnly()).isFalse();
    }
    ...
    void getPublishedOnly_everyStatus_isExhaustivelyMapped(ArticleStatusEnum status) {
        assertThat(new MyArticlePageParam().setStatus(status).getPublishedOnly()).as("%s 未映射", status).isNotNull();
    }

    @Test
    @DisplayName("不传排序时按创建时间倒序")
    void resolveSortField_notProvided_defaultsToCreateTime() {
        assertThat(new MyArticlePageParam().toPage().orders()).extracting(OrderItem::getColumn, OrderItem::isAsc)
            .containsExactly(org.assertj.core.api.Assertions.tuple(MyArticleSortEnum.CREATE_TIME.getColumn(), false),
    ...
    void sortEnums_publicAndAdmin_areSeparate() {
        ...
        assertThat(top.harrylei.bitlog.article.model.enums.ArticleSortEnum.values()).extracting(Enum::name)

...
public class CategoryServiceImpl implements CategoryService {
    ...
    public List<CategoryVO> listAll(String name) {
        ...
        return categories.stream()
                .map(cat -> articleConverter
                        .toCategoryVO(cat)
                        .setArticleCount(
                                countByCategory.getOrDefault(cat.getId(), 0L).intValue()))
                .sorted(Comparator.comparingInt(CategoryVO::getArticleCount).reversed())
                .toList();
    }
    ...
    public void update(Long categoryId, CategoryUpdateParam req) {
        ...
        String name = req.getName().trim();
        if (name.equals(category.getName())) return;
    ...
    public void delete(Long categoryId) {
        ...
        // 草稿及已删除文章保留内容，只解除分类引用；与 article.category_id 外键保持一致。
        articleDAO.clearUnpublishedCategory(categoryId);

...
public class AuthController {
    ...
    /**
     * 刷新 Token
     * <p>使用 HttpOnly Cookie 中的 Refresh Token 换取新的 Access Token，同时轮换 Refresh Token。
     */
    @PostMapping("/refresh")
    ...
    /**
     * 退出登录
     * <p>仅凭 HttpOnly Cookie 中的 Refresh Token 操作，无需 Access Token，
     * 确保 Access Token 已过期的用户也能正常退出并撤销会话。
     */
    @PostMapping("/logout")
    ...
    private void setRefreshTokenCookie(HttpServletResponse response, String refreshToken) {
        ResponseCookie cookie = ResponseCookie.from(REFRESH_TOKEN_COOKIE, refreshToken)
                ...
                .sameSite("Lax")
                .path(COOKIE_PATH)
                .maxAge(jwtProperties.getRefreshTokenExpire())
                .build();

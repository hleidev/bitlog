...
public interface UserFollowMapper extends BaseMapper<UserFollowDO> {
     ...
     * 分页查询用户关注列表
     */
    IPage<UserFollowVO> pageFollowingList(UserFollowPageQuery queryParam, IPage<UserFollowVO> page);

    /**
     * 分页查询用户粉丝列表
     */
    IPage<UserFollowVO> pageFollowersList(UserFollowPageQuery queryParam, IPage<UserFollowVO> page);
}

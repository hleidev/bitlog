...
/* Regular <img>: render at natural size, capped to viewport, centered. */
.img-lightbox__img {
  display: block;
  position: absolute;
  top: 50%;
...
/* HTML/SVG slot (Mermaid): fill the stage; SVG uses viewBox + object-fit
   to scale up to viewport while preserving aspect ratio (equivalent to
   preserveAspectRatio="xMidYMid meet" per MDN). */
.img-lightbox__html {
  position: absolute;
  inset: 0;
  display: flex;
...
/* Make the slot's direct child (e.g. CodeBlockView's .mermaid-svg-wrap)
   fill the stage so its SVG can scale by viewBox + max-* containment
   instead of collapsing to its intrinsic 300x150 default. */
.img-lightbox__html > * {
  display: flex;
  align-items: center;
  justify-content: center;

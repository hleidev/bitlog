...
function onCatEnter() {
  ...
  if (first && first.name.toLowerCase() === q.toLowerCase()) { selectCat(first); return }
  if (filteredCats.value.length === 1) { selectCat(filteredCats.value[0]); return }
...
async function createAndAddTag(name: string) {
  try {
    ...
    if (!tagOpts.value.find(t => t.id === id)) {
      tagOpts.value = [...tagOpts.value, { id, name, articleCount: 0, createTime: '', updateTime: '' }]
    }
    tagInput.value = ''
    tagOpen.value  = false
  } catch {
...
function setSummary(v: string) { internalSummary.value = v }
function setCategory(cat: { id: number; name: string }) { selectCat(cat) }
              ...
              <div class="pf-item">
                ...
                <div v-click-outside="closeCatDropdown" class="pf-combo">
                  ...
                  <div v-if="catOpen" class="pf-combo-dropdown">
                    ...
                    >{{ cat.name }}</button>
                    <button
                      v-if="catInput.trim() && !filteredCats.find(c => c.name.toLowerCase() === catInput.trim().toLowerCase())"
                      type="button"
                      class="pf-combo-opt"
                      @mousedown.prevent="createAndSelectCat(catInput.trim())"
                    >{{ catInput.trim() }}</button>
                ...
                <div v-click-outside="() => { tagOpen = false; tagInput = '' }" class="pf-combo">
                  <div class="pf-combo-field" :class="{ 'pf-combo-field--focused': tagOpen }">
                    <input
                      ...
                      @keydown.escape="tagOpen = false; tagInput = ''"
                  ...
                  <div v-if="tagOpen" class="pf-combo-dropdown">
                    ...
                    >{{ tag.name }}</button>
                    <button
                      v-if="tagInput.trim() && !tagOpts.find(t => t.name.toLowerCase() === tagInput.trim().toLowerCase())"
                      type="button"
                      class="pf-combo-opt"
...
.dialog-body { padding: 20px 24px 16px; overflow: visible; }
